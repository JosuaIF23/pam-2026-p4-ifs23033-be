...
fun Application.configureRouting() {
    ...
    val watchService: WatchService by inject()
    ...
    routing {
        get("/") {
            call.respondText("API telah berjalan. Dibuat oleh Josua Silalahi.")
        }
        ...
        // === ROUTE WATCHES (BARU) ===
        route("/watches") {
            get { watchService.getAllWatches(call) }
            post { watchService.createWatch(call) }
            get("/{id}") { watchService.getWatchById(call) }
            put("/{id}") { watchService.updateWatch(call) }
            delete("/{id}") { watchService.deleteWatch(call) }
            get("/{id}/image") { watchService.getWatchImage(call) }
        }
